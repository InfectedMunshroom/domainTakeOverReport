...
def getUrls(dork, number_webs, enable_save, filename):
    print(
        colors.OKCYAN
        + "Starting DorkScraper to recollect all the URLs that appear with the dork "
        + colors.FAIL
        + dork
        + colors.ENDC
    )
    ...
    try:
        for results in search(
            dork, tld="com", lang="es", num=number_webs, start=0, stop=None, pause=2
        ):
            print(results)
            time.sleep(0.2)
...
def automatedScan(dorklist, wordlist):
    with open(dorklist, "r") as name:
        for line in name:
            with open(wordlist, "r") as word:
                for link in word:
                    liner = line + ".txt"
                    dork = '"{}" site:{}'.format(line, link)
                    getUrls(dork, 10000, 1, liner)
    ...
    if len(sys.argv) == 1:
        ...
        if arg == "-h" or arg == "--help":
            ...
            print(
                "Usage:"
                + colors.OKCYAN
                + '\tdockerscraper.py "dork" number_of_websites'
                + colors.ENDC
            )
            print(
                "Example:"
                + colors.OKCYAN
                + '\tdockerscraper.py "inurl:admin" 5 -s output.txt'
                + colors.ENDC
            )
            print(
                "-d,--dork"
                + colors.OKCYAN
                + "\tSpecifies the dork to use in the tool"
                + colors.ENDC
            )
            ...
            print(
                "-s,--save"
                + colors.OKCYAN
                + "\tEnable save output and specifies the output file"
                + colors.ENDC
            )
